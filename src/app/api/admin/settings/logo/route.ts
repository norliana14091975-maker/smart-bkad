...
const BASE_NAME = 'app-logo'

/** Unggah logo aplikasi (tampil di sidebar dan header). */
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ data: { logoUrl: url } })
  } catch (error) {
...
/** Hapus logo kustom — kembali memakai emblem bawaan. */
export async function DELETE() {
  try {
    ...
    await db.appSetting.deleteMany({ where: { key: 'logoUrl' } })

    return NextResponse.json({ data: { ok: true } })
  } catch (error) {
    console.error('DELETE /api/admin/settings/logo error', error)
