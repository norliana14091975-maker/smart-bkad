...
Task ID: 27
Agent: Z.ai Code (main)
Task: "buat setupwizard" — Setup Wizard: panduan konfigurasi awal bertahap (dialog 5 langkah) yang terbuka otomatis saat login admin pertama kali

Work Log:
- Baca worklog Task 26 (Pengaturan AI Copilot provider LLM) + file terkait: admin-settings-section.tsx (pola seksi + draf copilot), copilot-providers.ts/copilot-config.ts (registry 11 provider + klien OpenAI-Compatible), settings.ts/default-settings.ts (AppSetting key-value), auth.ts (requireAdmin/getAdminUser), users.ts (resetUserPassword — pola keepSessionId), users API routes, page.tsx, dialog.tsx
- BARU src/lib/setup-wizard.ts (server): SETUP_WIZARD_KEY 'setupWizardCompleted' di app_setting; getSetupWizardStatus(admin) — completed/completedAt/username + 3 pemeriksaan live (identityConfigured: appTitle/govName ≠ bawaan DKI; passwordDefault: verifyPassword('admin123') pada akun yang login; copilotConfigured: provider ≠ default); markSetupCompleted()/resetSetupCompleted()
- BARU /api/admin/setup-wizard (GET status / POST tandai selesai / DELETE reset — semua requireAdmin) + BARU /api/admin/users/change-password (POST {currentPassword,newPassword}; getAdminUser semua role; validasi 8-72 kar tanpa spasi; verifikasi password lama; tolak sama dengan lama; hash baru + hapus sesi lain kecuali sesi aktif via SESSION_COOKIE)
- src/types/budget.ts: SetupWizardStatusDto (completed/completedAt/username/checks{identityConfigured,passwordDefault,copilotConfigured})
- BARU src/components/dashboard/setup-wizard.tsx: dialog 5 langkah (Pengantar → Identitas → Keamanan → AI Copilot → Selesai) — header gradien navy + stepper lingkaran berikon (selesai emerald ✓ / aktif navy / mendatangi muted, konektor berwarna mengikuti progres, label tersembunyi di mobile); langkah 0 checklist status live 3 baris + catatan bila pernah selesai; langkah 1 form identitas (appTitle/govName/brandText/brandSubtext/footerText terisi dari pengaturan, counter karakter, validasi wajib judul+pemda, simpan PUT /api/admin/settings); langkah 2 keamanan (peringatan amber bila masih admin123 / hijau bila sudah, form 3 password + toggle tampil, validasi klien + server, sukses → toast + lanjut otomatis, kolom kosong → tombol jadi "Lanjut" melewati); langkah 3 copilot (select 11 provider + model + key masker + baseUrl, praisi otomatis, Uji Koneksi pakai draf, default = cukup "Lanjut"); langkah 4 ringkasan live + petunjuk menu lanjutan + tombol "Selesaikan Setup" (POST) dan "Tutup tanpa menandai selesai"; footer navigasi Kembali/Lewati/Simpan & Lanjut responsif (stack di mobile); export hook useSetupWizardStatus(enabled) dipakai bersama kartu Pengaturan; reset ke langkah 0 saat dibuka + bersihkan field sensitif saat ditutup
- Integrasi page.tsx: state wizardOpen + ref wizardAutoChecked — setelah login admin (atau reload dengan sesi admin) GET /api/admin/setup-wizard sekali per sesi halaman, bila belum completed wizard terbuka OTOMATIS; logout mereset penanda cek; SetupWizard dirender hanya bila role=admin; AdminSettingsSection menerima prop onOpenWizard
- admin-settings-section.tsx: kartu "Setup Wizard" di paling atas Pengaturan — badge Selesai (emerald) / Belum selesai (amber), teks tanggal selesai (formatDateID) atau ajakan jalankan sekali, tombol "Jalankan Setup Wizard" (navy)
- Perbaikan saat verifikasi: max-w-2xl menimpa max-w-[calc(100%-2rem)] bawaan DialogContent (CSS order) sehingga dialog 390px full-viewport → diganti max-w-[min(42rem,calc(100%-2rem))] (358px + margin 16px di mobile)
- Verifikasi curl: GET anonim 401; status admin (identityConfigured=true Seruyan, passwordDefault=true, copilotConfigured=false); change-password — password lama salah 400, <8 kar 400, sama-dengan-lama 400, sukses 200 → login lama 401/baru 200 → direstorasi admin123; POST complete → completed=true + timestamp; DELETE reset → false; OPD 401
- Verifikasi agent-browser (desktop+mobile): login admin → WIZARD TERBUKA OTOMATIS dengan checklist benar; langkah identitas terisi nilai Kab. Seruyan; Simpan & Lanjut → keamanan; password salah → pesan error tampil; kolom kosong → Lanjut melewati; copilot default Z.ai → Uji Koneksi sukses 266 ms "OK"; Selesaikan Setup → dialog tutup; RELOAD → wizard TIDAK terbuka lagi (completed); Pengaturan → kartu badge "Selesai · 25 Agustus 2026" → Jalankan Setup Wizard terbuka kembali dengan catatan selesai; ganti password admin123→WizardDemo99 via wizard → lanjut otomatis ke langkah 4 → login password baru sukses → direstorasi; mobile 390px: dialog 358px tanpa overflow, footer nav stack, body scroll, semua langkah render; login kepala_daerah → TIDAK ada wizard (hanya admin); 0 error konsol (1 warning pre-existing DialogContent); dev.log semua 200; lint & tsc bersih (0 error file baru)
- State akhir DB: app_setting setupWizardCompleted terisi (wizard pernah diselesaikan — auto-open tidak terjadi lagi); password admin tetap admin123

Stage Summary:
- Setup Wizard lengkap: dialog 5 langkah dengan pemeriksaan status live (identitas, password bawaan, provider copilot), terbuka otomatis sekali untuk admin hingga ditandai selesai, dapat dijalankan ulang kapan saja dari kartu di Pengaturan Aplikasi
- API baru: /api/admin/setup-wizard (GET/POST/DELETE) + /api/admin/users/change-password (ganti password sendiri dengan verifikasi, sesi lain dihapus)
- Semua langkah opsional (identitas disimpan lewat API pengaturan yang sama; copilot memakai endpoint Task 26; keamanan via endpoint baru); tidak ada perubahan skema Prisma (memakai app_setting yang ada)
- 4 file baru (lib, 2 route API, komponen wizard) + 3 file diubah (types, page, settings section)

---
Task ID: 28
Agent: Z.ai Code (main)
Task: "ganti data asal Provinsi DKI Jakarta menjadi Kabupaten Seruyan dan lainnya. Perbaiki Setup wizard tidak berjalan ketika aplikasi pertama kali dijalankan dan hapus Akun default: admin / admin123 · kepala_daerah / kepala123" — Setup Wizard first-run (inisialisasi tanpa akun bawaan) + pembersihan seluruh data asal DKI Jakarta

Work Log:
- Baca worklog Task 26-27 + file terkait (default-settings, emblem, layout, login route/dialog, setup-wizard lib+komponen, admin-settings-section, copilot-config/route, users.ts, seed.ts, schema.prisma, page.tsx) dan state DB (5 user, app_setting Seruyan)
- GANTI DATA ASAL DKI → SERUYAN: default-settings.ts (appTitle 'Dashboard Keuangan Kab. Seruyan', govName 'Pemerintah Kabupaten Seruyan', brandText 'PEMDA', brandSubtext/footerText Kab. Seruyan); emblem.tsx (BrandLogo default PEMDA/Kab. Seruyan, teks lingkar GoldEmblem 'KABUPATEN SERUYAN'); layout.tsx keywords (Kabupaten Seruyan, PEMDA); placeholder admin-settings-section (Judul/Brand/Sub-teks); rename komponen DkiEmblem→DaerahEmblem & BpkdLogo→BrandLogo (header/sidebar/admin-settings); localStorage key 'bpkd.*'→'seruyan.*'; SESSION_COOKIE 'bpkd_admin_session'→'dashboard_admin_session' (sesi lama otomatis logout sekali)
- SETUP WIZARD FIRST-RUN (inti permintaan): src/lib/setup-wizard.ts — isFirstRunNeeded() (jumlah akun role admin = 0); getSetupWizardStatus disederhanakan (checks.identityConfigured kini = terisi & BEBAS pola legacy /dki|jakarta|bpkd/i — bukan lagi "beda dari bawaan"; checks.passwordDefault DIHAPUS karena tidak ada lagi password bawaan)
- API baru PUBLIK dengan guard: GET /api/setup/status (hanya {needed, appTitle}); POST /api/setup/complete (403 bila sudah ada admin; validasi username isValidUsername + unik, password 8-72 tanpa spasi, identitas wajib appTitle+govName + limit panjang, copilot opsional dengan validasi sama seperti Task 26; buat adminUser role admin → simpan identitas → simpan copilot → markSetupCompleted → createSession + cookie = AUTO-LOGIN); POST /api/setup/test-copilot (403 bila setup selesai; kirim draf provider ke testCopilotConnection)
- Komponen baru src/components/dashboard/first-run-setup.tsx: overlay layar-penuh fixed z-[100] TIDAK bisa ditutup (role=dialog aria-modal) — 5 langkah Pengantar → Akun Admin (username/password/konfirmasi + toggle tampil, validasi klien) → Identitas (IDENTITY_FIELDS pra-terisi Seruyan, diekspor dari setup-wizard.tsx bersama StatusRow) → AI Copilot (11 provider, Uji Koneksi via endpoint first-run) → Selesai (ringkasan + 'Selesaikan Setup & Masuk' → POST complete → onFinished); header gradien navy + stepper sama seperti wizard admin; password dibersihkan dari memori setelah sukses
- page.tsx: state firstRunNeeded (null=loading, fetch /api/setup/status saat mount) → render FirstRunSetup overlay bila true; handleFirstRunFinished → set user + invalidate cache settings/setup-wizard/copilot + langsung ke admin-overview
- Wizard admin (dialog lama) disesuaikan: langkah Keamanan kini generik "ganti password bila perlu" (hapus semua pesan admin123/password bawaan); checklist langkah 0 & 4 tinggal Identitas + Copilot; SetupWizardStatusDto.checks tanpa passwordDefault
- login-dialog.tsx: HAPUS paragraf "Akun default: admin/admin123 · kepala_daerah/kepala123"; login route: pesan khusus 403 "Belum ada akun pengguna…" bila total user = 0
- prisma/seed.ts DIBANGKITKAN ULANG dari data riil DB produksi (skrip scripts/generate-seed.ts): 3 OPD (upsert per kode — aman seed ulang, akun OPD pengguna tidak terhapus), 3 APBD 2024-2026, 47 item anggaran, 840 baris realisasi (global TA 2024/2025 + opd TA 2026 via lookup kode→id), 2+2 SKPD, 31 dokumen — SEMUA angka Seruyan (sebelumnya angka DKI Jakarta); TANPA akun bawaan
- DB berjalan: hapus akun default admin & kepala_daerah (sesi ter-cascade) → 3 akun OPD tersisa; backup /tmp/backup-pre-task28.db (pra-perubahan) & /tmp/task28-final-state.db (state akhir)
- Verifikasi curl: status needed=true saat tanpa admin; validasi 400 (username pendek, password pendek, identitas kosong, provider tak dikenal, model kosong); happy path 200 + cookie auto-login; setelah setup complete/test-copilot 403; login akun baru 200; login admin/admin123 lama 401; status wizard admin completed=true
- Verifikasi agent-browser: first-run wizard muncul otomatis menutup dashboard → Mulai Setup → akun admin-seruyan → identitas pra-terisi Seruyan → Uji Koneksi Z.ai 268 ms OK → Selesaikan Setup → AUTO-LOGIN ke Ringkasan Admin (menu Admin + tombol Keluar muncul); reload → wizard tidak muncul lagi; logout → dialog login TANPA hint akun default; kartu Setup Wizard di Pengaturan badge "Selesai" + wizard admin 5 langkah dengan checklist baru; wizard first-run mobile 390px full-width tanpa overflow (VLM: layout bersih, tanpa teks terpotong)
- Simulasi INSTALASI BARU total: hentikan dev server (kill proses next lama yang memegang inode DB terhapus — penyebab error SQLITE_READONLY_DBMOVED saat write), db/custom.db baru → db:push → seed Seruyan baru (0 user, 840 realisasi) → /api/setup/status needed=true → POST complete sukses → admin overview menampilkan data seed; state akhir pengguna direstorasi (840 realisasi, 3 OPD, 0 admin)
- Verifikasi final: admin-final dibuat via wizard E2E penuh (dengan cookie baru dashboard_admin_session) lalu dihapus → first-run wizard aktif kembali untuk pemilik aplikasi; lint & tsc bersih (0 error file baru); dev.log hanya 200/401 yang diharapkan

Stage Summary:
- Setup Wizard kini BERJALAN saat aplikasi pertama kali dijalankan: begitu belum ada akun admin, wizard layar penuh wajib diisi (buat akun admin pertama + identitas Seruyan + copilot opsional) dan auto-login setelah selesai — tidak ada lagi akun bawaan admin/admin123 & kepala_daerah/kepala123 (dihapus dari seed & DB berjalan)
- Seluruh data asal DKI Jakarta diganti Kabupaten Seruyan: nilai bawaan identitas, emblem/logo, metadata, placeholder, kunci penyimpanan, nama cookie, dan data seed (angka APBD/realisasi riil Seruyan hasil ekspor DB produksi)
- Keamanan endpoint first-run: /api/setup/complete & /api/setup/test-copilot hanya berlaku selama belum ada admin (403 setelahnya); API key copilot tetap tidak pernah dikirim utuh ke klien
- 4 file baru (3 route API setup + komponen FirstRunSetup + skrip generator seed) + 12 file diubah; tanpa perubahan skema Prisma
- State akhir DB: 3 akun OPD aktif, 0 admin → wizard first-run AKTIF; pemilik aplikasi membuat akun admin sendiri saat pertama membuka aplikasi
